\ audio.fth
\ audio driver for CS5530 based on openfirmware driver and BSD native
...
: init-buffers  ( -- )
   ...
   /bufs dma-alloc drop to dma-in-phys 
   /bufs dma-alloc drop to dma-out-phys

   \ allocate PRDs
   /prds dma-alloc to prd-in  to prd-in-phy  
   /prds dma-alloc to prd-out to prd-out-phy 
...
;
\ FIXME initiate dma transfer on channel one 
...
: poll-dma ( --- ) \ check transfer status
;
...
: mulaw   ( -- )
   ...
   ulaw8mono to sample>
;
: default ( -- )  stereo 48khz disable-playback  set-vra ;
...
: start-geode-dma ( channel# in? -- )
    ...
    over if dma-in-phy else dma-out-phy then
...
: stop-geode-dma ( channel#  --  ) 
    >dma-regs
    dup >dma-status c@ drop          \ DMA Status, Paranoid Read clears EOP flgs
    >dma-cmd dup c@ h# 0fe and   \ Clear enable bit
    swap c!                         \ Write Clear enable bit. can corrupt PCI state?
    0 to geode-dma-poll             \ Stop polling if EOT or EOP!!!!.
;

previous definitions

base !
.( sequence :  open if init-audio then     )
