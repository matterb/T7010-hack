...
[defined] tr-flag [if] trace-on [then]
...
: init-usb-dev  ( -- )
debug? if cr ." init-usb-dev" then
   set-device 
   device di-act-config@    to configuration
...
\ fload ${BP}/dev/usb2/device/storage/atapi.fth
...
0 value scsi-open-count

\ external
: open-scsi  ( -- flag )
debug? if cr ." open-scsi " then
   scsi-open-count  if
      reopen-hardware  dup  if  scsi-open-count 1+ to scsi-open-count  then
      exit
   else
      open-hardware  dup  if
         1 to scsi-open-count
...
;
: close-scsi  ( -- )
   scsi-open-count 1- to scsi-open-count
   scsi-open-count  if
      reclose-hardware
   else
      close-hardware
...
: show-children  ( -- )
   open-scsi  0=  if  ." Can't open SCSI host adapter" cr  exit  then
...
: get-scsi-type  ( lun -- false | type true )
   open-scsi  0=  if  2drop false exit  then
   set-address inquiry
...
: parent-max-transfer  ( -- n )  s" max-transfer"  type 100 ;
: parent-set-address  ( lun -- )  set-address ;
...
: unit-ready?  ( -- ready? )
   test-unit-rdy-cmd  no-data-command  0=
;
...
0 value msd-open-count

: open  ( -- flag ) \ do a one time initialize with init-msd 
 ...
 open-scsi
      ...
      d# 2000 set-timeout
   then
   msd-open-count 1+ to msd-open-count
   true
;

: close ( -- )
  msd-open-count 1- 0 max to msd-open-count
  close-scsi
;
...
\ testing
\ init-msd show-children open close
