...
debug? if cr ." init-atapi-execute-command" then
...
debug? if cr ." open-scsi" then
   msd-open-count  if
      reopen-hardware  dup  if  msd-open-count 1+ to msd-open-count  then
      exit
   else
...
: parent-set-address  ( lun -- )  scsi-set-address ;
...
: open  ( -- flag )
   my-unit parent-set-address      \ my-unit = lun ?
