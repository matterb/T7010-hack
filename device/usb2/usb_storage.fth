\ usb_storage.fth
hex
also usbdefs defnitions
[defined] system [if]  also system
  [defined] usbdefs [if] also usbdefs
      [else] definitions vocabulary usbdefs definitions
  [then]
...
s" h# 72 cmos@ 1+ dup .d h# 72 cmos!" evaluate
cr
[then]
...
: retry-command?  ( dma-addr dma-len dma-dir cmd-addr cmd-len #retries -- actual errcode )
   s" retry-command?" $call-parent
;


\ Simplified command execution routines for common simple command forms

: no-data-command  ( cmdbuf -- error? )  s" no-data-command" $call-parent  ;

: short-data-command  ( data-len cmdbuf cmdlen #retries -- true | buffer len false )
   s" short-data-command" $call-parent
;
